	...
	for snap == nil {
		...
		if number == 0 {
			...
			if checkpoint != nil {
				...
				state, err := chain.StateAt(hash)
				if err != nil {
					return nil, err
				}

				// get validators and current span
				validators, err := c.GetCurrentValidators(
					number+1,
					state,
					checkpoint,
					chainContext{Chain: chain, Bor: c},
				)
				if err != nil {
					return nil, err
				}
...
func (c *Bor) Prepare(chain consensus.ChainHeaderReader, header *types.Header) error {
	...
	if (number+1)%c.config.Sprint == 0 {
		state, err := chain.StateAt(header.Hash())
		if err != nil {
			return err
		}

		newValidators, err := c.GetCurrentValidators(number+1, state, header, chainContext{
			Chain: chain,
			Bor:   c,
		})
		if err != nil {
...
func (c *Bor) GetCurrentSpan(
	...
	result, err := fetchMessage(msg, state, header, c.chainConfig, chCtx)
	if err != nil {
		return nil, err
	}
...
func (c *Bor) GetCurrentValidators(
	blockNumber uint64,
	state *state.StateDB,
	header *types.Header,
	chCtx core.ChainContext) ([]*Validator, error) {
	// method
...
func (c *Bor) checkAndCommitSpan(
	...
	headerNumber := header.Number.Uint64()
	span, err := c.GetCurrentSpan(state, header, chain)
	if err != nil {
		return err
	}
...
func (c *Bor) CommitStates(
	...
	number := header.Number.Uint64()
	_lastStateID, err := c.GenesisContractsClient.LastStateId(state, header, chain)
	if err != nil {
		return nil, err
	}
